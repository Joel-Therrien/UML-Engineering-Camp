...
char *streaming_tokens[NUM_TRACES] = {"bk9v1hl4gu", "xhaie2ymyg"}; //Enter the tokens you generated under Plotly API settings

plotly graph = plotly("Joel_Therrien", "f9d2ruzaog", streaming_tokens, "weather", NUM_TRACES); //Create a plotly graph with your username, API key, streaming tokens, and graph name
...
void loop()
{
  ...
  graph.plot(x, P, streaming_tokens[0]); //Add a timestamped temperature data point to the graph
  graph.plot(x, a, streaming_tokens[1]); //Add a timestamped humidity data point to the graph
//    graph.plot(x, lightVal, streaming_tokens[2]); //Add a timestamped light data point to the graph
//    graph.plot(x, uvVal, streaming_tokens[3]); //Add a timestamped UV data point to the graph
